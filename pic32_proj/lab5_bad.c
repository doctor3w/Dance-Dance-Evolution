/*
 * File:        lab5.c
 * Author:      Drew Dunne & Michael Solomentsev
 * Adapted from:
 *              main.c by
 * Author:      Bruce Land
...
#define SwapSRAM(sram) (sram == BIT_0 ? BIT_1 : BIT_0)
const int SRAM_1 = BIT_0, SRAM_2 = BIT_1;

volatile int w_addr = 0x0;
volatile int r_addr = 0x0;
volatile int cur_addr = 0x0; // Applies to both SRAMs (r/w)
volatile int w_sram = BIT_0;
volatile int r_sram = BIT_1;
volatile char r_valid = 0;

volatile short adc_read;
volatile short ram_read;
...
void __ISR(_TIMER_2_VECTOR, ipl2) Timer2Handler(void)
{
  ...
  int value2 = ram_read_byte(cur_addr);
  dac_write_byte(value2<<4);
  cur_addr++;
  //ram_write_short(w_addr, adc_read >> 2);
//  if (r_valid) {
//    ram_read = ram_read_short(w_addr);
//    dac_write_short(ram_read << 4); // Writes to channel A
//  }
//  
//  w_addr += 2;
//  r_addr +=2;
...
int main(void)
{
  ...
  ram_write_byte_array(cur_addr, test_data, 8);
  int i;
  ...
  // === now the threads ====================
  // TODO: setup input polling thread/scoring thread
  // TODO: setup beat processing thread (maybe in ISR)

  // schedule the threads
  while(1) {
    // round robin
    // TODO: setup thread schedule
